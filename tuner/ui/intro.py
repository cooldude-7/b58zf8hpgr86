...
The brand arrives over the top of that, in three parts: the lambda
fades up once the surface has built, the wordmark then slides out from
behind it to the right revealed by a moving edge rather than flown in, and
the speed-density equation fades in beneath the pair.
...
FORM_CAP = 0.21          # formula cap height, in mark heights
FORM_DROP = 0.30         # formula top below the baseline, in mark heights
...
class IntroOverlay(QWidget):
    ...
    def _brand(self):
        """The mark, the wordmark and the equation, loaded once. Any may be
        missing -- a build made before they existed -- so the caller falls
        back to setting the name as text rather than drawing nothing."""
        if self._mark is None:
            self._mark = QPixmap(str(asset("brand-mark.png")))
            self._word = QPixmap(str(asset("brand-word.png")))
            self._form = QPixmap(str(asset("brand-formula.png")))
    ...
    def brand_frame(self, w, h, t_ms=None):
        ...
        if mark.isNull() or word.isNull() or self._form.isNull():
            return None
        ...
        # The equation hangs under the lockup, left-aligned with the mark.
        # Scaled by its cap height rather than its image height, so the
        # letters match the wordmark's size however the art is regenerated.
        fh = mh * FORM_CAP * self._form_cap_ratio()
        fw = fh * self._form.width() / max(self._form.height(), 1)
        form_rect = QRectF(x, base + mh * FORM_DROP, fw, fh)
        return (mark_in, mark_rect, _ease((t - WORD_AT_MS) / WORD_IN_MS), word_rect,
                base, _ease((t - FORM_AT_MS) / FORM_IN_MS), form_rect)
    ...
    def _paint_brand(self, p, w, h):
        ...
        if state is None:
            return None
        mark_in, mark_rect, word_in, word_rect, base, form_in, form_rect = state
        if mark_in <= 0.0:
            return int(base)
        ...
        if word_in > 0.0:
            ...
            p.restore()
        if form_in > 0.0:
            # The equation only fades: wiping it as well would read as the
            # same gesture twice, and it is the quiet line of the three.
            p.setOpacity(form_in)
            p.drawPixmap(form_rect, self._form, QRectF(self._form.rect()))
            p.setOpacity(1.0)
        p.restore()
        return int(base)
    ...
    @staticmethod
    def _form_cap_ratio():
        """The equation image's height as a multiple of its cap height.

        Its tallest ink is the numerator of a stacked fraction and its
        lowest the denominator, so the image is a good deal taller than a
        capital: 228 units against a cap of 100 when the art was authored."""
        return 2.28
    ...
    def _paint_text(self, p, w, h):
        ...
        if base is None:
            ...
            # The equation is art, so without it say the same thing in words
